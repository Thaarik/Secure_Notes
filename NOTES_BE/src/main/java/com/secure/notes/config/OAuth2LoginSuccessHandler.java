...
public class OAuth2LoginSuccessHandler extends SavedRequestAwareAuthenticationSuccessHandler {
    ...
    // Method to find custom logic after authentication success
    @Override
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws ServletException, IOException {
        OAuth2AuthenticationToken oAuth2AuthenticationToken = (OAuth2AuthenticationToken) authentication; // casting to OAuth2AuthenticationToken type
        if ("github".equals(oAuth2AuthenticationToken.getAuthorizedClientRegistrationId()) || "google".equals(oAuth2AuthenticationToken.getAuthorizedClientRegistrationId())) {
            DefaultOAuth2User principal = (DefaultOAuth2User) authentication.getPrincipal(); //This is how the user is represented when using OAuth2
            Map<String, Object> attributes = principal.getAttributes();
            String email = attributes.getOrDefault("email", "").toString(); //defaultValue is used when the value is not available
            String name = attributes.getOrDefault("name", "").toString();
            // GitHub and Google attributes might differ. so the extraction also differs
            ...
            userService.findByEmail(email)
                    .ifPresentOrElse(user -> {
                        // if present, we are updating the security context with the existing user details
                        ...
                        //if not present
                        // new user with default role or existing role
